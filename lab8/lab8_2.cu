...
__global__ void left_right_bc(double *data, int x, double bc, int nX, int nY) {
...
__global__ void up_down_bc(double *data, int y, double bc, int nX, int nY) {
...
//---------------------------------------------------------------------------------// Выделение памяти (n+2 из-за дополнительных ячеек)
	...
	cudaMalloc(&gpu_data, sizeof(double) * rez);
	cudaMalloc(&gpu_next, sizeof(double) * rez);
	data = (double*)malloc(sizeof(double) * rez);
	buff = (double*)malloc(sizeof(double) * b_size);
	double* output = (double *)malloc(sizeof(double) * rez);
	unsigned long long size = n_size * nX * nY;
//---------------------------------------------------------------------------------// Определение рзмер буфера


    buff = (double*)malloc(sizeof(double) * b_size);
    cudaMalloc(&gpu_buff, sizeof(double) * b_size);
    int buffer_size;
...
//---------------------------------------------------------------------------------// Инициализаця блока	
    ...
    cudaMemcpy(gpu_data, data, sizeof(double) * rez, cudaMemcpyHostToDevice);
...
//---------------------------------------------------------------------------------// Отправка данных
	while (true) {
		...
		if (ib + 1 < nb_X) {
            ...
            cudaMemcpy(buff, gpu_buff, sizeof(double) * b_size, cudaMemcpyDeviceToHost);
...
//---------------------------------------------------------------------------------// Прием данных
		...
		if (ib > 0) {
            ...
            cudaMemcpy(gpu_buff, buff, sizeof(double) * b_size, cudaMemcpyHostToDevice);
...
//---------------------------------------------------------------------------------// Вывод днных
	cudaMemcpy(output, gpu_data, sizeof(double) * rez, cudaMemcpyDeviceToHost);
