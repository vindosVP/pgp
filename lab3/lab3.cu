...
__constant__ matrix dev_cov[32];
...
__device__ __host__ vec3 getRGB(uchar4* pixel) {
    vec3 result;
    result.x = pixel->x;
    result.y = pixel->y;
    result.z = pixel->z;
...
__global__ void mahalanobis_kernel(uchar4* img,
    ...
    for (int x = idx; x < height; x += offset_x) {
        for (int y = idy; y < width; y += offset_y) {
...
int main() {
    ...
    CSC(cudaMemcpyToSymbol(dev_avg, avgs, 32 * sizeof(vec3)));
    CSC(cudaMemcpyToSymbol(dev_cov, covs, 32 * sizeof(matrix)));
    ...
    mahalanobis_kernel<<<dim3(32, 32), dim3(16, 16)>>>(out_img, width, height, nc);
