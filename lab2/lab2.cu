...
__device__ uint8_t findmedian(uint16_t *arr, uint16_t size) {
    uint16_t tmp = 0;
    uint8_t res = 0;
    for (uint16_t i = 0; i < (uint32_t) 256; i++) {
        tmp += arr[i];
        if (tmp > size / 2) {
            res = (uint8_t) i;
            break;
...
__device__  findingdots(uint16_t r_arr, uint16_t g_arr, uint16_t b_arr, pos now_pos, pos begin, pos end){
    uint16_t f = 0;
...
__device__ uint32_t FindMedDot(uint32_t h, uint32_t w,
                                   pos begin, pos end) {
    uint16_t r_arr[(uint32_t) 256];
    uint16_t g_arr[(uint32_t) 256];
    uint16_t b_arr[(uint32_t) 256];
    for (uint16_t i = 0; i < (uint16_t) (uint32_t) 256; i++) {
        r_arr[i] = 0;
        g_arr[i] = 0;
        b_arr[i] = 0;
    }
    ...
    uint16_t f = 0;
    f = findingdots(r_arr, g_arr, b_arr, pos now_pos, pos begin, pos end);

    return MakeDot(findmedian(count_array_red, f),
                     findmedian(count_array_green, f),
                     findmedian(count_array_blue, f), 0);
}

__device__ void findlastdot(pos pos, uint32_t radius, uint32_t h, uint32_t w,
                            uint32_t *map) {
    pos begin, end;
    ...
    map[truepos(pos, height, width)] = FindMedDot(h, w, begin, end);
}
